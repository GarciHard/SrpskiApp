 ...
 */
public class VOptionPane extends JDialog implements ActionListener, MouseListener {
    ...
    public VOptionPane(JDialog parentFrame, String message) {
        initFrame(parentFrame, message);
    }

    private void initFrame(JDialog parentFrame, String message) {
        //Creación del JDialog suplente de JOptionPane
        ...
        pnlBackground.setBackground(new Color(51,153,51));
        pnlBackground.setBorder(BorderFactory.createLineBorder(new Color(255,255,255),2));

        //Creacion del JLabel para el logo de los mensajes
        lblLogo = new JLabel();
        lblLogo.setSize(new Dimension(100, 100));
        if (message.compareTo("Empleado Agregado") == 0) {
          lblLogo.setIcon(new ImageIcon(this.getClass().getResource("/images/CheckmarkWhiteIcon.png")));
        } else {
          lblLogo.setIcon(new ImageIcon(this.getClass().getResource("/images/ErrorWhiteIcon.png")));
        }
        lblLogo.setLocation(0, 0);

        //Creacion del JLabel de título xD
        lblAnuncio = new JLabel("Mensaje de la base de datos", SwingConstants.LEFT);
